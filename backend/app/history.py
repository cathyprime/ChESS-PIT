...
def history_game_json(game: Game, bot_id: int) -> dict:
    ...
    return {
        ...
        "opponentId": game.black_bot_id if bot_is_white else game.white_bot_id,
        "opponentName": game.black_name if bot_is_white else game.white_name,
...
def bot_history_page(db, bot: Bot, offset: int, limit: int) -> dict:
    ...
    return {"total": total, "games": [history_game_json(game, bot.id) for game in games]}
